...
import traceback
import redis
from django_interface_test_platform.settings import redis_port
...
def data_handler(global_key, requestData):
    try:
        if re.search(r"\$\{unique_num\d+\}", requestData):  # 匹配用户名参数，即"${www}"的格式
            ...
            var_name = var_name.split("_")[1]  #['unique', 'num']
            print("var_name: %s" % var_name)
        ...
        if re.search(r"\$\{(\w+)\}", requestData):
            ...
            for var_name in re.findall(r"\$\{(\w+)\}", requestData):
                ...
                print("json.loads(os.environ[global_key])[var_name]: {}".format(json.loads(os.environ[global_key])[var_name]))
    ...
    try:
        if '&&' in key_word:
            ...
            for keyWord in key_word_list:
                # 如果断言词非空，则进行断言
                if keyWord:
                    print("json.dumps(responseObj.json())): {}".format(json.dumps(responseObj.json(), ensure_ascii=False)))
                    # 没查到断言词则认为是断言失败
