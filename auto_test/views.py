...
def testcase(request):
    ...
    if request.method == "GET":
        ...
        server_address = get_server_address(env)  # 增加的代码
        ...
        if test_case_id_list:
            ...
            print("用例获取到，开始用例的执行")
            tasks.run_test_case_task(test_case_id_list, server_address)  # 增加的代码
        else:
            print("运行测试用例失败")
...
def module_testcases(request, module_id):
    ...
    return render(request, 'testcase.html', {'testcases': get_paginator(request, testcases)})  #用回之前的模板'testcase.html'
...
def test_case_execute_record(request):
    ...
    return render(request, 'testCaseExecuteRecord.html', {'testCaseExecuteRecords': get_paginator(request, testCaseExecuteRecords)})
...
def suit_case_execute_record(request, suit_record_id):
    ...
    suit_case_execute_records = models.TestSuitTestCaseExecuteRecord.objects.filter(test_suit_record=test_suit_execute_record)
    return render(request, 'suitCaseExecuteRecord.html',{'suit_case_execute_records': get_paginator(request, suit_case_execute_records)})
